    ...
    for (unsigned int jx = 0; jx < num_div; ++jx) {
      for (unsigned int jy = 0; jy < num_div; ++jy) {
        if (abs_diff(ix, jx) <= 1 && abs_diff(iy, jy) <= 1) { // this grid is near to the particle as the grid indexes are close
          ...
          // Assume that a large particle is at the center of the gravity of the particles in a far grid.
          
          unsigned int num_particles_in_grid = acc.grid2idx[jy * num_div + jx + 1] - acc.grid2idx[jy * num_div + jx];
          particles[ip].force += num_particles_in_grid * gravitational_force(acc.grid2cg[jy * num_div + jx] - particles[ip].pos);
        }
      }
    }
  }
